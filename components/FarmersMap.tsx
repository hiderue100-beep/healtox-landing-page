      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          ...
          <p className="text-base text-primary-muted max-w-md">
            HEALTOX는 수입산 저가 찻잎 대신, 대한민국 각 지역을 대표하는 장인 농가와 직접 계약재배하여 100% 투명한 원료를 공급받습니다.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-5 glass-card rounded-3xl p-8 bg-surface-DEFAULT border border-surface-border shadow-elevated relative min-h-[440px] flex flex-col justify-between">
            <div className="flex items-center justify-between mb-6">
              <span className="text-xs font-bold uppercase tracking-wider text-botanical-700 bg-botanical-100 px-3 py-1 rounded-full">
            ...
            <div className="relative w-full h-64 my-auto bg-white rounded-2xl border border-surface-border p-4 flex items-center justify-center overflow-hidden">
              ...
              <div className="relative w-full h-full max-w-[260px]">
                {FARMS.map((farm) => {
                  ...
                  // Map approximate relative positions
                  ...
                  return (
                    <button
                      ...
                      className={`absolute -translate-x-1/2 -translate-y-1/2 p-2 rounded-full transition-all duration-300 group z-20 ${
                        isSelected
                          ? "bg-botanical-500 text-white ring-4 ring-botanical-500/30 scale-125 shadow-glow"
                          : "bg-primary-DEFAULT text-white hover:bg-botanical-600 hover:scale-110"
                      }`}
                      title={farm.region}
                    >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                    selectedFarm.id === f.id
                      ? "bg-botanical-600 text-white shadow-subtle"
          ...
          <div className="lg:col-span-7">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="glass-card rounded-3xl p-8 sm:p-10 bg-white border border-surface-border shadow-elevated"
              >
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                  <div>
                    <span className="inline-block px-3 py-1 rounded-full bg-botanical-100 text-botanical-700 text-xs font-bold uppercase mb-2">
                      {selectedFarm.tag}
                    </span>
                    <h3 className="text-2xl sm:text-3xl font-bold text-primary-DEFAULT">
                      {selectedFarm.region}
                    </h3>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-sm font-bold text-primary-DEFAULT flex items-center gap-1.5">
                      <UserCheck className="w-4 h-4 text-botanical-500" />
                      {selectedFarm.farmer}
                    </div>
                  </div>
                </div>

                <div className="p-4 rounded-xl bg-surface-subtle border border-surface-border mb-6">
                  <div className="text-xs font-bold text-primary-subtle uppercase mb-1">재배 원료 & 토양 특성</div>
                  <div className="text-base font-bold text-botanical-700 mb-1">{selectedFarm.crop}</div>
                ...
                <blockquote className="p-4 rounded-xl bg-botanical-50/60 border border-botanical-200 text-xs sm:text-sm text-primary-DEFAULT italic mb-6">
                  "{selectedFarm.quote}"
                </blockquote>
                ...
                <div className="pt-6 border-t border-surface-border flex flex-wrap items-center justify-between gap-4 text-xs text-primary-subtle">
                  <div className="flex items-center gap-4">
                    <span className="flex items-center gap-1">
                      <ShieldCheck className="w-4 h-4 text-botanical-500" /> 100% 잔류 농약 검사 완료
                    </span>
                    <span className="flex items-center gap-1">
                      <HeartHandshake className="w-4 h-4 text-botanical-500" /> 공정 무역 직접 계약
                    </span>
                  </div>
                  <a
                    href="#find-my-tea"
                    className="font-bold text-botanical-600 hover:text-botanical-700 flex items-center gap-1 group"
                  >
                    이 농가 원료 수분 블렌드 찾기
