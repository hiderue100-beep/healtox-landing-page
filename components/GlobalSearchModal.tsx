...
import { Search, X, Leaf, Sparkles, HelpCircle, ArrowRight } from "lucide-react";

interface SearchResult {
  type: "Ingredient" | "Tea" | "FAQ";
  title: string;
  subtitle: string;
  link: string;
}
          ...
          >
            ...
            <div className="relative mb-6">
              ...
              <input
                ...
                autoFocus
                className="w-full pl-12 pr-10 py-4 rounded-2xl bg-surface-subtle border border-surface-border text-primary-DEFAULT placeholder-primary-subtle text-sm font-medium focus:outline-none focus:border-orange-500 focus:bg-white transition-all shadow-inner"
              />
              {query && (
                <button
                  onClick={() => setQuery("")}
            ...
            <div className="flex items-center gap-2 mb-4 text-xs font-bold text-primary-subtle">
              <span className="text-orange-600 font-extrabold uppercase">Quick Searches:</span>
              {["박하", "감잎", "동백꽃", "수면", "부종"].map((tag) => (
                <button
                  key={tag}
                  onClick={() => setQuery(tag)}
                  className="px-2.5 py-1 rounded-md bg-surface-subtle hover:bg-orange-50 hover:text-orange-600 transition-colors"
                >
                  #{tag}
                </button>
              ))}
                ...
                results.map((item, idx) => {
                  ...
                  return (
                    <a
                      ...
                      href={item.link}
                      onClick={onClose}
                      className="p-3.5 rounded-xl bg-white hover:bg-orange-50/60 border border-surface-border hover:border-orange-200 transition-all flex items-center justify-between group"
                    >
                      <div className="flex items-center gap-3">
                        <span className="w-8 h-8 rounded-lg bg-orange-100 text-orange-800 flex items-center justify-center shrink-0">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-bold text-primary-DEFAULT group-hover:text-orange-600 transition-colors">
                              {item.title}
                            </span>
                          </div>
                      ...
                      <ArrowRight className="w-4 h-4 text-primary-subtle group-hover:text-orange-600 group-hover:translate-x-1 transition-transform" />
                    </a>
                  );
                })
              ) : (
